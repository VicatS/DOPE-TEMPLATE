<div class="container">
	...
	<table class="table table-bordered table-responsive" style="margin-top: 20px;">
		<thead>
			<tr>
				<td>ID</td>
				<td>Nombre Completo Empleado</td>
				<td>Dirección</td>
      ...
      <div class="modal-body">
        	<form id="myForm" action="" method="post" class="form-horizontal">
        		...
        		<div class="form-group">
        			<label for="name" class="label-control col-md-4">Nombre Empleado</label>
        			<div class="col-md-8">
        				<input type="text" name="txtEmployeeName" class="form-control">
        			</div>
        		</div>
        		<div class="form-group">
        			<label for="address" class="label-control col-md-4">Dirección</label>
        			<div class="col-md-8">
...
<script>
	$(function(){
		...
		$('#btnAdd').click(function(){
			$('#myModal').modal('show');
			$('#myModal').find('.modal-title').text('Agregar nuevo empleado');
				...
				$.ajax({
					...
					success: function(response){
						if(response.success){
							...
							$('.alert-success').html('Empleado '+type+' satisfactoriamente').fadeIn().delay(4000).fadeOut('slow');
							showAllEmployee();
						}else{
							alert('Error');
						}
				...
				$.ajax({
					...
					success: function(response){
						if(response.success){
							...
							$('.alert-success').html('Empleado eliminado satisfactoriamente').fadeIn().delay(4000).fadeOut('slow');
							showAllEmployee();
						}else{
							alert('Error');
						}
					...
					for(i=0; i<data.length; i++){
						...
						html +='<tr>'+
									...
									'<td>'+
										...
										'<a href="javascript:;" class="btn btn-danger item-delete" data="'+data[i].id+'">Eliminar</a>'+
									'</td>'+
							    '</tr>';
					}
					$('#showdata').html(html);

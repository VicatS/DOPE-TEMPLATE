...
$(document).ready(function () {
    ...
    $(".btn-view-categoria").on("click", function(){
     ...
     // ver accesorios de modelo
    $(document).on("click",".btn-view-modelo",function(){
        valor_id = $(this).val();
        $.ajax({
            url: base_url + "mantenimiento/modelos/view/",
            type:"POST",
            dataType:"html",
            data:{id:valor_id},
            success:function(data){
      ...
      // ver accesorios de modelo
    $(document).on("click",".btn-agregar-accesorio",function(){
        valor_id = $(this).val();
        $.ajax({
            url: base_url + "mantenimiento/modelos/add",
            type:"POST",
            dataType:"html",
            data:{id:valor_id},
            success:function(data){
    ...
    //ver articulo
       ...
       //ver accesorio
       $(".btn-view-accesorio").on("click", function(){
        var accesorio = $(this).val(); 
       
        var infoaccesorio = accesorio.split("*");
        html = "<p><strong>Código: </strong>" + infoaccesorio[0]+"</p>"
        html += "<p><strong>Modelo: </strong>" + infoaccesorio[1]+"</p>"
        html += "<p><strong>Calidad: </strong>" + infoaccesorio[2]+"</p>"
        html += "<p><strong>Color: </strong>" + infoaccesorio[3]+"</p>"
        html += "<p><strong>Precio: </strong>" + infoaccesorio[4]+"</p>"
        html += "<p><strong>Stock: </strong>" + infoaccesorio[5]+"</p>"
        html += "<p><strong>Categoria: </strong>" + infoaccesorio[6]+"</p>"
        html += "<p><strong>Marca: </strong>" + infoaccesorio[7]+"</p>"
        html += "<p><strong>Descripción: </strong>" + infoaccesorio[8]+"</p>"
        $("#modal-default .modal-body").html(html);
    });
    ...
    // para recuperar articulos para un autocomplete segun modelo
       $("#articulo").autocomplete({
        ...
        select:function(event, ui){
            data = ui.item.idAccesorio + "*"+ ui.item.idModelo+ "*"+ ui.item.label+ "*"+ ui.item.precio+ "*"+ ui.item.stock;
    ...
    $("#btn-agregar").on("click",function(){
        ...
        if (data !='') {
            ...
            html = "<tr>";
            html += "<td><input type='hidden' name='idAccesorio[]' value='"+infoarticulo[0]+"'>"+infoarticulo[1]+"</td>";
            html += "<td>"+infoarticulo[2]+"</td>";
            html += "<td><input type='text' name='precios[]' value='"+infoarticulo[3]+"' class='precios'></td>";
    ...
    $("#btn-agregar-servicio").on("click",function(){
     ...
     // para recuperar articulos para un autocomplete segun modelo
       $("#accesorio").autocomplete({
        source:function(request, response){
            $.ajax({
                url: base_url+"movimientos/pedidos/get_accesorios",
                type: "POST",
                dataType:"json",
                data:{ valor: request.term},
                success:function(data){
        ...
        select:function(event, ui){
            data = ui.item.idAccesorio + "*"+ ui.item.idModelo+ "*"+ ui.item.label+ "*"+ ui.item.precio+ "*"+ ui.item.stock;
            $("#btn-agregar-pedidos").val(data);
        },
    });


     //para agregar accesorios para tabla pedidos
    $("#btn-agregar-pedidos").on("click",function(){
        data = $(this).val();
        if (data !='') {
            infoaccesorio = data.split("*");
            html = "<tr>";
            html += "<td><input type='hidden' name='idAccesorio[]' value='"+infoaccesorio[0]+"'>"+infoaccesorio[0]+"</td>";
            html += "<td>"+infoaccesorio[2]+"</td>";
            html += "<td><input type='hidden' name='precios[]' value='"+infoaccesorio[3]+"' class='precios'>"+infoaccesorio[3]+"</td>";
            html += "<td>"+infoaccesorio[4]+"</td>";
            html += "<td><input type='text' name='cantidades[]' value='1' class='cantidades'></td>";
            html += "<td><input type='hidden' name='importes[]' value='"+infoaccesorio[3]+"'><p>"+infoaccesorio[3]+"</p></td>";
            html += "<td><button type='button' class='btn btn-danger btn-remove-articulo'><span class='fa fa-remove'></span></button></td>";
            html += "</tr>";
            $("#tbpedidos tbody").append(html);
            sumarPedido();
            $("#btn-agregar-pedidos").val(null);
            $("#accesorio").val(null);
        }else{
            $('.error').fadeIn().delay(1000).fadeOut('slow');
        }
    ...
    $(document).on("keyup","#tbventas input.cantidades", function(){
     ...
     // columnda de detalle ventas para tabla-precios
    $(document).on("keyup","#tbventas input.precios", function(){
     ...
     // columnda de detalle servicio-cantidades
    $(document).on("keyup","#tbordenes input.cantidades", function(){
      ...
      // columnda de detalle servicio-precios
    $(document).on("keyup","#tbordenes input.precios", function(){
        precio = $(this).val();
     ...
     // columnda de detalle pedidos para tabla
    $(document).on("keyup","#tbpedidos input.cantidades", function(){
       cantidad = $(this).val();
        precio = $(this).closest("tr").find("td:eq(2)").children("input").val();
        importe = cantidad * precio;
        $(this).closest("tr").find("td:eq(5)").children("p").text(importe.toFixed(2));
     ...
     // columnda de detalle pedidos para tabla-precios
    $(document).on("keyup","#tbpedidos input.precios", function(){
        precio = $(this).val();
        cantidad= $(this).closest("tr").find("td:eq(4)").children("input").val();
        importe = cantidad * precio;
        $(this).closest("tr").find("td:eq(5)").children("p").text(importe.toFixed(2));
    ...
    $("input[name=total]").val(total.toFixed(2));
      ...
      //funcion para sumar el total de un pedido

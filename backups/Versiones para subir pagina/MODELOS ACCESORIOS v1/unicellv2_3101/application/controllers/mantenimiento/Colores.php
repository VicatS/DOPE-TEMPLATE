...
class Colores extends CI_Controller {
	...
	public function store(){
		$codigo_hexadecimal=$this->input->post("codigo_hexadecimal");
		$nombre_color=$this->input->post("nombre_color");
		$idusuario = $this->session->userdata("id");

		//validaciones 
		$this->form_validation->set_rules("codigo_hexadecimal","Color","required|is_unique[color.codigoHexadecimal]|trim");
		$this->form_validation->set_rules("nombre_color","Nombre Color","required|alpha|is_unique[color.nombreColor]|trim");
		if ($this->form_validation->run() == TRUE) {
			$data = array(
				'codigoHexadecimal' =>$codigo_hexadecimal,
	...
	public function update(){
		...
		if ($codigo_hexadecimal==$color_Actual->codigoHexadecimal) {
			$uniqueCod = '';
		}else{
			$uniqueCod='|is_unique[color.codigoHexadecimal]';
		}
		$this->form_validation->set_rules("codigo_hexadecimal","Color","required".$uniqueCod);//concatenoamos regla de validacion
		$this->form_validation->set_rules("nombre_color","Nombre Color","required|alpha".$unique);//concatenoamos regla de validacion
		...
		if ($this->form_validation->run()) {
			...
			if ($this->Colores_model->update($id_color,$data)) {
				...
				$this->session->set_flashdata("error","No se pudo guardar la información");
				redirect(base_url()."mantenimiento/colores/edit/".$id_color);
			}
		}else{
			$this->edit($id_color);
		}
